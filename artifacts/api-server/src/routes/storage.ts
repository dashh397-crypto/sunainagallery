...
router.post(
  ...
  async (req: Request, res: Response) => {
    ...
    if (
      ...
      !parsed.data.contentType.startsWith('video/')
    ) {
      res.status(400).json({ error: 'Only image and video files are supported' });
      return;
    }
  ...
  try {
    ...
    const [media] = vault
      ? await db
          ...
          .where(
            and(
              eq(mediaItemsTable.vaultId, vault.id),
              eq(mediaItemsTable.objectPath, objectPath),
            ),
          )
          .limit(1)
      : [];
